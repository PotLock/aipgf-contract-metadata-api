...
export async function ExtractRustMetadata({ sourceCode, account, methods }: any) {
  const { text } = await generateText({
    ...
    system:
      ...
      ` returns [{ account:${account}, method: ${methods}  , description : description with method 100 words limit , args <only show if it has args>: [ name: Argument name , type : data types ,description }} ]   `,
